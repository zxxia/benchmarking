...
vs_path = '/home/zxxia/Projects/benchmarking/plot/e2e_results_30s_10s/awstream_e2e_results_{}.csv'
for video in VIDEOS:
    filename = vs_path.format(video)
    videos, perfs, accs = load_awstream_results(filename)
...
plt.legend()
plt.title('awstream on static vs moving ')
plt.figure()
...
plt.figure()
videos, perfs, accs = load_awstream_results(
    '../awstream/awstream_e2e_waymo.csv')
plt.scatter(perfs_to_plot, accs_to_plot, s=PS, label='our')
