...
def main():
	...
	data_path = '/Users/zhujunxiao/Desktop/benchmarking/vldb/data/'
	feature_file = data_path + 'features_all_type_width_height_filter.csv'
	...
	path = data_path + 'awstream/spatial_overfitting_profile_09_28/'
	...
	for key in sorted(awstream_perf.keys()):
		...
		if features[key][all_feature_names.index('velocity_avg')] < 1 or \
			...
			continue
		# if the total_object_cn is small, the result is not robust. 
		# one object could easily baise the f1 score
		...
		ax.scatter(features[key][all_feature_names.index(feature1)], 
					awstream_perf[key][2], c='b')
		ax.text(features[key][all_feature_names.index(feature1)], 
					awstream_perf[key][2], key)
	# plt.xlim([0,0.1])
	plt.show()
	print(keys)
