...
video_to_delete = ['nyc', 'russia', 'taiwan', 'crossroad', 'crossroad2', 'driving_downtown',
	...
	for file in glob.glob(path+'awstream_*.csv'):
		...
		with open(file, 'r') as f:
			...
			for line in f:
				...
				perf_dict[key] = float(line_list[3]) 

				if line_list[1] != '720p':
					continue
				else:
					total_object_cn[line_list[0]] = int(line_list[4])
	return total_object_cn, perf_dict
...
def main():
	...
	feature2 = 'object_size_median'
	feature3 = 'velocity_avg'
	...
	path = '/Users/zhujunxiao/Desktop/benchmarking/vldb/data/awstream/spatial_overfitting_profile_10_08/'
	total_object_cn, perf_dict = profile_parser(path)
	...
	tmp5 = []
	# ax.set_xscale('log')
	for target_dataset in ['driving2', 'highway', 'motorway', 'crossroad', 'jp', 'park']:
		for i in range(0, 30):
			key = target_dataset + '_' + str(i)

			if key in features and key in total_object_cn:
				if total_object_cn[key] < 300:
					continue
				tmp.append(features[key][all_feature_names.index(feature1)])
				tmp2.append(features[key][all_feature_names.index(feature2)])
				tmp3.append(perf_dict[(key, '540p')])
				tmp4.append(perf_dict[(key, '480p')])
				tmp5.append(perf_dict[(key, '360p')])
			else:
				print('{} missing'.format(key))

	ax[0].plot(tmp, c='b', label=feature1)
	ax[0].plot(tmp2, c='r', label=feature2)
	ax[1].plot(tmp3, c='k', label='f1 for 540p')
	ax[1].plot(tmp4, c='b', label='f1 for 480p')
	ax[1].plot(tmp5, c='r', label='f1 for 360p')
	plt.legend()
	plt.show()
