...
from my_utils import nms, Most_Common
...
def tag_object(all_filename, frame_to_object, output_folder):
	...
	with open(output_file, 'w') as f:
		f.write('Frame ID, Object ID, X, Y, W, H, Object Type\n')
		for filename in all_filename:
			boxes = frame_to_object[filename]
			current_frame = []
			for box in boxes:
				assert len(box) == 5, print(filename, boxes)
				...
				if not last_frame:
					...
					for last_box in last_frame:
						dist = np.linalg.norm(np.array([x_c, y_c]) - 
							np.array([last_box[0], last_box[1]]))
...
def smooth_annot(all_filename, frame_to_object):
	...
	for filename in all_filename:
		if not frame_to_object[filename - 1]  \
		   or not frame_to_object[filename + 1]:
			...
			for box in next_boxes:
				...
				if not current_boxes:
					...
					for current_box in current_boxes:
						...
						dist = np.linalg.norm(np.array([x_c, y_c]) - 
							np.array([current_x_c, current_y_c]))
...
def read_annot(annot_path, height_thresh):
	...
	with open(annot_path, 'r') as f:
		f.readline()
		for line in f:
			# each line: (image_name, bounding boxes (x, y, w, h, object_type))
			...
			for index in range(len(boxes_list)):
				...
				dets[index, 2] = x + w
				dets[index, 3] = y + h
				dets[index, 4] = 1 # forget to output score, use 1 for every box. Fix this later
				tmp[frame_id].append([x, y, w, h, t])
			keep = nms(dets, 0.4)
			for index in keep:
				# if t == 1: # object is 'car'
				box_list = boxes_list[index].split(' ')
				if int(box_list[3]) >= height_thresh:
					frame_to_object[frame_id].append([int(x) for x in box_list])
...
def main(argv):
	...
	if FLAGS.resize:
		...
		image_resolution = [int(x) for x in FLAGS.resol.split(',')]#image_resolution_dict[video_index]
	...
	new_frame_to_object = smooth_annot(all_filename, 
									   frame_to_object)
