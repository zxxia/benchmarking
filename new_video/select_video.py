...
def main():
    features_names = ['Object Count', 'Object Area',
                      'Object Velocity', 'Percent of Frame with Object']  #'Arrival Rate','Total Object Area',
    video_clips, obj_cnt, obj_area, arrival_rate, obj_velocity, tot_obj_area, \
    percent_frame_w_obj, similarity = load_video_features('features_all_30s.csv')
    print('features file has {} items.'.format(len(video_clips)))


    vs_file = '/home/zxxia/benchmarking/videostorm/figure7/videostorm_30s_overfitting_all.csv'
    vs_video_list, vs_perf_list, vs_acc_list = load_performance(vs_file)
    print('videostorm result file has {} items.'.format(len(vs_video_list)))


    # Code for basic statistics of the original feature values
    features = [obj_cnt, obj_area,  obj_velocity, percent_frame_w_obj]#arrival_rate,tot_obj_area,

    print(video_clips[np.argmax(obj_area)])

    plt.plot(percent_frame_w_obj)
    plt.show()
    ...
    for i in range(len(video_clips)):
        ...
        for j in range(len(video_clips)):
            ...
            if np.abs(obj_cnt[j] - obj_cnt[i]) <= (1 * features_range[0]) and \
               np.abs(obj_area[j] - obj_area[i]) <= (0.1 * features_range[1]) and \
               np.abs(obj_velocity[j] - obj_velocity[i]) <= (0.025 * features_range[2]) and \
    ...
    with open('tmp.csv', 'w') as f:
        for idx, result in enumerate(results):
            ...
            print(idx)
            for i in result:
                ft.append(percent_frame_w_obj[i])
                perf.append(vs_perf_list[i])
                f.write('\''+video_clips[i]+ '\' ,')
