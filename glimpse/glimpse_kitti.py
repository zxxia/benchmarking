...
def write_pipeline_result(frame_start, frame_end, resize_rate, 
...
def eval_pipeline_accuracy(frame_start, frame_end, resize_rate, gt_annot, dt_glimpse, iou_thresh=0.5):
	tp = defaultdict(int)
	fp = defaultdict(int)
	fn = defaultdict(int)
	gt_cn = 0
...
def pipeline(img_path, dt_annot, gt_annot, frame_start, frame_end, csvf, 
	...
	tracking_error_thresh, resize_rate=1, kitti=True):
	...
	for i in range(frame_start + 1, frame_end + 1):
		...
		if frame_diff > frame_difference_thresh:
			...
			lastTriggeredFrameGray = oldFrameGray.copy()
			frame_flag[i] =  1
		else: # use tracking to get the result
			assert last_index in dt_glimpse, print(last_index)
			if not dt_glimpse[last_index] : # last frame is empty, trigger 
				# dt_glimpse[i] = dt_annot[i].copy()
				# triggered_frame += 1
				# oldFrameGray = newFrameGray.copy()
				# lastTriggeredFrameGray = oldFrameGray.copy()
				...
				oldFrameGray = newFrameGray.copy()
				frame_flag[i] =  0
			else: # need to use tracking to get detection results
				...
				for [x, y, w, h, t] in dt_glimpse[last_index]: #dt_annot[i - 1]:						
					...
					if old_corners is not None:
						...
						old_corners = np.append(old_corners, [[[np.float32(x) , np.float32(y)]]], axis=0)

					else:
						old_corners = [[[np.float32(x) , np.float32(y)]]]
					old_corners = np.append(old_corners, [[[np.float32(x+w) , np.float32(y)]]], axis=0)
					old_corners = np.append(old_corners, [[[np.float32(x) , np.float32(y+h)]]], axis=0)
					old_corners = np.append(old_corners, [[[np.float32(x+w) , np.float32(y+h)]]], axis=0)
