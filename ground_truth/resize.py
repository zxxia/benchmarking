...
def resize_video(video_in, video_out, target_size):
    cmd = "ffmpeg -y -i {} -loglevel panic -hide_banner -vf scale={} {}".format(video_in,
                                                                target_size,
                                                                video_out)
    print(cmd)
    os.system(cmd)
...
def extract_frames(video, output_path):
    cmd = "ffmpeg -y -i {} {}%06d.jpg -loglevel panic -qscale:v 2.0 -hide_banner".format(video, output_path)
    print(cmd)
    os.system(cmd)
...
def main(argv):
    ...
    #                 box = box_str.split(' ')
    #                 box[0] = int(int(box[0]) * x_scale)
    #                 box[1] = int(int(box[1]) * y_scale)
    #                 box[2] = int(int(box[2]) * x_scale)
    #                 box[3] = int(int(box[3]) * y_scale)
