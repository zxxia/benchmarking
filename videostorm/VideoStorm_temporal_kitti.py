...
from utils.utils import IoU, interpolation
from utils.model_utils import load_ssd_detection

#TODO: there dependency bugs needed to be fixed. e.g. function naming
def main():
        iou_thresh = 0.5
        ...
        for video_name in video_index_dict.keys():
            for video_index in video_index_dict[video_name]:
                ...
                full_model_dt, gt, img_list = load_full_model_detection(fullmodel_detection_path)
                F1_list = []
                ...
                # for sample_rate in [20,10,5,2,1]:
                        ...
                        for boxA in current_gt:
                            ...
                            if iou_list:    
                                if max(iou_list) >= iou_thresh:
                                    tp += 1
                                    dt_boxes_final.remove(dt_boxes_final[iou_list.index(max(iou_list))])                                                         
                                    flag = 1
                            if not flag:
                                fn += 1
                        # for boxA in current_gt:
                    ...
                    print("True positive:{0}, False positive: {1}, False negative: {2}".format(tp, fp, fn))
                ...
                if value[-1] == 0:
                    ...
                    index = next(x[0] for x in enumerate(result_vec) if x[1] > target_f1)
                    if index == 0:
