...
def smooth_label(label_dict, thresh=0.7, area_thresh=0.02, person_thresh=1000/(600*400)):
	...
	label_to_delete = ['umbrella', 'trafficlight', 'kite', 
						...
						'clock', 'laptop', 'stopsign', 'refrigerator','keyboard',
						'mouse', 'surfboard',  'frisbee',  'pottedplant', 'cat',  
						'toothbrush', 'baseballbat', 'bear',  'broccoli',  'remote',
						'carrot',  'sink', 'sheep','wineglass', 'handbag', 
						'backpack', 'horse', 'vase','knife']
	...
	for i in sorted(label_dict.keys()):
		...
		if len(label_dict[i]) == 1:
			# no confidence score and area information can be used for filterings
			new_dict[i] = [label]
			continue
		...
		box = label_dict[i][3]
		if label == 'person':
			# person will have smaller area, therefore should use smaller filtering
			# thresh (person_thresh)
...
def eval(annot1, annot2, dataset, f):
	...
	for i in range(1, 32000):
		...
		if len(annot1[i]) > 2:
			area[i//(short_video_length*frame_rate)].append(float(annot1[i][1]))
...
def main():
	...
	'''
	The classification label converted from detection results is very very very
	noisy. Need to do smoothing.
	'''
	...
	for dataset in ['crossroad4_3']:
		...
		thresh2 = thresh_dict[dataset][1] # relative area thresh, remove small boxes
		smoothed_fasterrcnn_label = smooth_label(fasterrcnn_label, thresh1, thresh2, person_thresh)

		# save smoothed labels
		with open(filtered_ground_truth, 'w') as f:
			for i in range(1, 32000):
				f.write(str(i) + ',' + ','.join(smoothed_fasterrcnn_label[i]) + '\n')
		...
		smoothed_mobilenet_label = smooth_label(mobilenet_label, thresh1, thresh2, person_thresh)
		...
		with open(filtered_mobilenet_label_path, 'w') as f:
			for i in range(1, 32000):
				f.write(str(i) + ',' + ','.join(smoothed_mobilenet_label[i]) + '\n')
