...
def compress_images_to_video_v1(path: str, start_frame: int, nb_frames: int,
    ...
    '''
    cmd = ['ffmpeg', '-r', str(frame_rate), '-f', 'image2', '-s',
           str(resolution), '-start_number', str(start_frame), '-i',
           '{}%06d.jpg'.format(path), '-vframes', str(nb_frames), '-vcodec',
           'libx264', '-crf', str(quality), '-pix_fmt', 'yuv420p',
