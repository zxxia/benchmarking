"""AWStream Interface offline simulation driver."""
import csv
import os
...
def run(args):
    ...
    pipeline = Awstream(sample_step_list, resolution_list, qp_list, profile_filename, output_path, awstream_temporal_flag, awstream_spacial_flag)
    with open(output_filename, 'w', 1) as f_out:
        ...
        for seg_path in seg_paths:
            ...
            for resol in resolution_list:
                video = dataset_class(seg_path, seg_name, resol, 'faster_rcnn_resnet101', filter_flag=True, classes_interested=classes_interested)
                videos[resol] = video
            ...
            for i in range(nb_short_videos):
                ...
                end_frame = min((i + 1) * short_video_length * original_video.frame_rate,
                ...
                best_resol, best_fps, best_bw = pipeline.Server(clip, videos, original_video,
                                                                [profile_start, profile_end])
                print('Profile {}: best resol={}, best fps={}, best bw={}'.format(clip, best_resol, best_fps, best_bw))
                ...
                f1_score, relative_bw = pipeline.evaluate(clip, original_video, videos[str(best_resol[1])+'p'], best_fps, [test_start, test_end])
